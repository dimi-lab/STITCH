...
integrated_obj <- RunPCA(integrated_obj, verbose = F)
...
integrated_obj <- FindClusters(integrated_obj, resolution = opt$resolution, cluster.name = "seurat_clusters_sketch")
...
integrated_obj <- RunUMAP(integrated_obj, reduction = paste0(opt$integration_method, ".full"), dims = 1:30, reduction.name = "umap.full",reduction.key = "UMAPfull_", return.model = TRUE)
