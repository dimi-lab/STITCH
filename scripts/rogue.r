...
ent.toli_fix <- function(ent, expr, n = 2, span = 0.5, r = 1, mt.method = "fdr"){
  sig.gene <- ent %>% dplyr::filter(p.adj < 0.05) %>% dplyr::pull(Gene)
  ng <- length(sig.gene)
  if(length(ng) >=2 ) expr <- expr[sig.gene,]
...
#'
#' @examples
rogue_fix <- function(expr, labels, samples, platform = NULL, k = NULL, min.cell.n = 10, remove.outlier.n = 2, span = 0.5, r = 1, filter = F, min.cells = 10, min.genes = 10, mt.method = "fdr"){
  ...
  sample.rogue <- function(meta, cluster){
    ...
    for (i in 1:length(s)) {
      ...
      if(length(index1) >= min.cell.n){
        ...
        ### fix issue when no significant genes can be found
        tmp.res <- ent.toli_fix(tmp.res, tmp.matr, span = span, r = r, n = remove.outlier.n)
