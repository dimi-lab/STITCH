...
m <- grid.arrange(grobs=p,ncol=round(sqrt(length(p))),nrow=ceiling(length(p)/round(sqrt(length(p)))))
ggsave('./marker_gene.png',m,width=12*round(sqrt(length(p))),height=12*ceiling(length(p)/round(sqrt(length(p)))),limitsize = F,dpi=72)
...
ggsave('./diff_gene.png',m,width=featureplot_width,height=featureplot_height,limitsize=F,dpi=72)
ggsave('./diff_gene.pdf',m,width=featureplot_width,height=featureplot_height,useDingbats=F,limitsize=F)
```

```{r diff_gene_featureplot1,eval=diff_flag, echo=FALSE}
