...
gp <- ggplot() + geom_col(aes(x=Var1, y=value,fill=Var2),position = position_dodge(), data=temp) + 
...
gp <- ggplot() + geom_col(aes(x=Var1, y=pct,fill=Var2),position = position_dodge(), data=temp) + 
  theme_classic()+ scale_x_continuous(breaks=0:(length(unique(temp[[1]]))-1)) + 
  theme(legend.title = element_blank(), legend.text = element_text(size=15), axis.text = element_text(size=15), axis.title = element_text(size=18,face="bold")) + 
  labs(x='cluster number', y='percentage of cells')
...
p <- DimPlot(seurat_obj, reduction = opt$vismethod, label=TRUE)
...
p <- DimPlot(seurat_obj, reduction = opt$vismethod, split.by = "sampleid",label=T, ncol = round(sqrt(nsamples)))
...
p <- DimPlot(seurat_obj, reduction = opt$vismethod, split.by = "condition",label=T, ncol = round(sqrt(nconditions)))
...
p <- DimPlot(seurat_obj, reduction = opt$vismethod, group.by = "Phase", split.by = "condition",label=T, ncol = round(sqrt(nconditions)))
...
if(ncol(sampleinfo) >3){
  for(i in 4:ncol(sampleinfo)){
    p <- DimPlot(seurat_obj, reduction = opt$vismethod,group.by = colnames(sampleinfo)[i] ,label=T)
...
genes <- genes$V1
if(nconditions ==1){
  p <- FeaturePlot(seurat_obj,features=genes,cols=c('grey','red'),combine = F,reduction = opt$vismethod, keep.scale = "feature", label = TRUE)
  ...
  m <- FeaturePlot(seurat_obj,features=genes,cols=c('grey','red'),combine = TRUE,reduction = opt$vismethod, keep.scale = "feature", split.by = "condition", label = TRUE) & theme(legend.position = "right")
...
p <- lapply(1:length(genelist_top1_gs), function(i) FeaturePlot(seurat_obj, features = genelist_top1_gs[i],reduction = opt$vismethod, label = TRUE) + labs(title = labels[i]))
m <- grid.arrange(grobs=p,ncol=round(sqrt(length(p))),nrow=ceiling(length(p)/round(sqrt(length(p)))))
...
p <- DotPlot(seurat_obj, features = rev(unique(genelist_top1_gs)), dot.scale = 8) + RotatedAxis()
...
#seurat_obj_heatmap <- ScaleData(seurat_obj_heatmap, features = rownames(seurat_obj_heatmap), assay = "SCT", verbose = FALSE)
...
png('./marker_gene_heatmap.png', width = 8, height = 12, res = 300)
...
pdf('./marker_gene_heatmap.pdf',width = 8, height = 12,useDingbats = FALSE)
ht$plot
invisible(dev.off())
...
ht <- FeatureHeatmap(
  srt = seurat_obj_heatmap, group.by = "seurat_clusters", split.by = "condition",max_cells = max(table(seurat_obj_heatmap$seurat_clusters)),features = unique(unlist(genelist_top5_gs)),assay = assay
)
ggsave('./marker_gene_heatmap_top5.png',ht$plot,width=8,height=12, dpi = 300)
ggsave('./marker_gene_heatmap_top5.pdf',ht$plot,width=8,height=12,useDingbats=FALSE)
...
p <- lapply(1:length(genelist_top5_gs),function(i) {
  p1 <- FeaturePlot(seurat_obj, features = genelist_top5_gs[[i]],combine = F,reduction = opt$vismethod, label = TRUE)
...
p <- DotPlot(seurat_obj, features = rev(unique(unlist(genelist_top5_gs))), cols = c("blue", "red"), dot.scale = 8) + RotatedAxis()
...
p <- lapply(1:length(genelist_top1_gs), function(i) FeaturePlot(seurat_obj, features = genelist_top1_gs[i],cols=c('grey','red'),combine = TRUE,reduction = opt$vismethod, keep.scale = "feature", split.by = "condition", label = TRUE) + theme(legend.position = "right") + scale_y_continuous(sec.axis = dup_axis(name = labels[i])))

m <- wrap_plots(p, ncol = 1)
diff_gene_featureplot_feature_width <- 6*nconditions
diff_gene_featureplot_feature_height <- 6*length(genelist_top1_gs)

ggsave('./diff_gene.png',m,width=diff_gene_featureplot_feature_width,height=diff_gene_featureplot_feature_height,limitsize=F,dpi=300)
ggsave('./diff_gene.pdf',m,width=diff_gene_featureplot_feature_width,height=diff_gene_featureplot_feature_height,useDingbats=F,limitsize=F)
```

```{r diff_gene_featureplot1, fig.width=diff_gene_featureplot_feature_width, fig.height=diff_gene_featureplot_feature_height,eval=diff_flag, echo=FALSE}
grid.draw(m)
```
...
png('./diff_gene_heatmap.png', width = 8, height = 12, res = 300)
ht <- FeatureHeatmap(
  srt = seurat_obj_heatmap, group.by = "seurat_clusters", split.by = "condition",max_cells = max(table(seurat_obj_heatmap$seurat_clusters)),features = unique(genelist_top1_gs),assay = assay
)
ht$plot
invisible(dev.off())

pdf('./diff_gene_heatmap.pdf',useDingbats = FALSE, width = 8, height = 12)
ht$plot
invisible(dev.off())
...
ggsave('./diff_gene_heatmap_top5.png',ht$plot,width=8,height=12,dpi = 300)
ggsave('./diff_gene_heatmap_top5.pdf',ht$plot,width=8,height=12,useDingbats=FALSE)
labels_top5 <- lapply(1:length(selectedclusters),function(i) paste0( selectedclusters[i],':',genelist_top5_gs[[i]]))
labels_top5 <- labels_top5[sapply(genelist_top5_gs,function(i) !any(is.na(i)))]
genelist_top5_gs <- genelist_top5_gs[sapply(genelist_top5_gs,function(i) !any(is.na(i)))]
## violin plot
...
* [violin plot](./figures/marker_gene_violin_top5_split.png)

For marker gene list table: [marker_genes](./marker_gene/gene_table.xlsx)

For filtered marker gene list table based on supplied cutoffs: [marker_genes_filter](./marker_gene/gene_table_filter.xlsx)
...
* [violin plot](./figures_diff_gene_violin_top5_split.png)

For DEGs within each cluster (case vs control): [diff_genes](./diff_gene/gene_table.xlsx)

For filtered DEGs within each cluster (case vs control): [diff_genes_filter.xlsx](./diff_gene/gene_table_filter.xlsx)
