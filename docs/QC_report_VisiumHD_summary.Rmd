...
## Pct of bins failing each QC metric with supplied cutoff

```{r}
gp <- ggplot(aes(x= name, y = value, color = name,label = sampleid), data = sample_metrics_long) + geom_boxplot() + theme_classic() + labs(x = "", y = "% of bins fail a certian metric") + geom_point(position = position_jitter(height = 0)) + theme(axis.text.x = element_text(angle = 45, vjust = 0.5))
remove_boxplot_outliers(ggplotly(gp))
```

## Pct of bins failing each QC metric with adaptive cutoff

```{r}
if(opt$adaptive_cutoff == "1"){
  gp <- ggplot(aes(x= name, y = value, color = name,label = sampleid), data = sample_metrics_long_postQC) + geom_boxplot() + theme_classic() + labs(x = "", y = "% of bins fail a certian metric") + geom_point(position = position_jitter(height = 0)) + theme(axis.text.x = element_text(angle = 45, vjust = 0.5))
remove_boxplot_outliers(ggplotly(gp))
...
gp <- ggplot(aes(x= name, y = value, color = name,label = sampleid), data = qc_summary_list_long) + geom_boxplot() + geom_point(position = position_jitter(height = 0)) + theme_classic() + coord_flip() + labs(x = "", y = "# of bins passing QC") + guides(fill="none") + theme(axis.text = element_text(size = 10), axis.title = element_text(size = 12, face = "bold"))
remove_boxplot_outliers(ggplotly(gp))
```
