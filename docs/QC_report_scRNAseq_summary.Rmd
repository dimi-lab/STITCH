...
sample_metrics_pct <- do.call(rbind,lapply(qc_list, function(i) {
  data.frame(sampleid = unique(i$sampleid),
             nCount_RNA = sum(i$nCount_RNA < opt$nCount_cutoff)/nrow(i),
             nFeature_RNA = sum(i$nFeature_RNA < opt$nFeature_cutoff)/nrow(i),
             percent.mt = sum(i$percent.mt > opt$mt_cutoff)/nrow(i),
             percent.hb = sum(i$percent.hb > opt$hb_cutoff)/nrow(i),
             percent.doublet = sum(i$scDblFinder.class == 'doublet')/nrow(i),
             percent.ambientRNA = unique(i$soupX.rho))
}))
...
sample_metrics_pct_postQC <- do.call(rbind,lapply(1:length(qc_list), function(i) {
  data.frame(sampleid = unique(qc_list[[i]]$sampleid),
             nCount_RNA = sum(qc_list[[i]]$nCount_RNA < opt_postqc_list[[i]]$nCount_cutoff)/nrow(qc_list[[i]]),
             nFeature_RNA = sum(qc_list[[i]]$nFeature_RNA < opt_postqc_list[[i]]$nFeature_cutoff)/nrow(qc_list[[i]]),
             ...
             percent.doublet = sum(qc_list[[i]]$scDblFinder.class == 'doublet')/nrow(qc_list[[i]]),
             percent.ambientRNA = unique(qc_list[[i]]$soupX.rho))
}))
...
sample_metrics_median <- do.call(rbind,lapply(qc_list, function(i) {
  apply(i[,c("nCount_RNA", "nFeature_RNA", "percent.mt", "percent.hb")], 2, median)
}))
...
for(i in c("nCount_RNA", "percent.mt", "percent.hb")) sample_metrics_median[,i] <- round(sample_metrics_median[,i], digits = 2)
...
There are a total of `r length(qc_metrics_summary_paths)` samples (total `r sum(ncells)` # of cells). The range of # of cells is `r paste0(min(ncells), '-', max(ncells))`. The range of median UMI counts is `r paste0(min(sample_metrics_median[,"nCount_RNA"]),"-",max(sample_metrics_median[,"nCount_RNA"]))`. The range of median detectable features is `r paste0(min(sample_metrics_median[,"nFeature_RNA"]),"-",max(sample_metrics_median[,"nFeature_RNA"]))`. The range of median mitochondrial (MT) concentration is `r paste0(min(sample_metrics_median[,"percent.mt"]),"-",max(sample_metrics_median[,"percent.mt"]), "%")`. The range of median hemoglobin (Hb) concentration is `r paste0(min(sample_metrics_median[,"percent.hb"]),"-",max(sample_metrics_median[,"percent.hb"]), "%")`. The range of sample-level total number of captured genes is `r paste0(min(qc_summary_list$total_genes), "-", max(qc_summary_list$total_genes))`
...
## Pct of ambient RNA content and doublet rate

```{r}
gp <- ggplot(aes(x= name, y = value, color = name,label = sampleid), data = sample_metrics_long[sample_metrics_long$name %in% c("percent.doublet", "percent.ambientRNA"),]) + geom_boxplot(outlier.shape = NA, outlier.colour = NA) + theme_classic() + labs(x = "", y = "% of cells") + geom_point(position = position_jitter(height = 0)) + theme(axis.text.x = element_text(angle = 45, vjust = 0.5))
remove_boxplot_outliers(ggplotly(gp))
```
...
## Pct of cells failing each QC metric with supplied cutoff

```{r}
gp <- ggplot(aes(x= name, y = value, color = name,label = sampleid), data = sample_metrics_long[!sample_metrics_long$name %in% c( "percent.ambientRNA"),]) + geom_boxplot() + theme_classic() + labs(x = "", y = "% of cells fail a certian metric") + geom_point(position = position_jitter(height = 0)) + theme(axis.text.x = element_text(angle = 45, vjust = 0.5))
remove_boxplot_outliers(ggplotly(gp))
```

## Pct of cells failing each QC metric with adaptive cutoff

```{r}
if(opt$adaptive_cutoff == "1"){
  gp <- ggplot(aes(x= name, y = value, color = name,label = sampleid), data = sample_metrics_long_postQC[!sample_metrics_long_postQC$name %in% c( "percent.ambientRNA"),]) + geom_boxplot() + theme_classic() + labs(x = "", y = "% of cells fail a certian metric") + geom_point(position = position_jitter(height = 0)) + theme(axis.text.x = element_text(angle = 45, vjust = 0.5))
remove_boxplot_outliers(ggplotly(gp))
...
gp <- ggplot(aes(x= name, y = value, color = name,label = sampleid), data = qc_summary_list_long) + geom_boxplot() + geom_point(position = position_jitter(height = 0)) + theme_classic() + coord_flip() + labs(x = "", y = "# of cells passing QC") + guides(fill="none") + theme(axis.text = element_text(size = 10), axis.title = element_text(size = 12, face = "bold"))
remove_boxplot_outliers(ggplotly(gp))
```
